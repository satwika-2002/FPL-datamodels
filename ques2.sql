select w.location,i.quantity,(i.quantity * p.base_price) as total_value from warehouses w inner join inventory i on w.warehouse_id = i.warehouse_id inner join products p on p.product_id = i.product_id
